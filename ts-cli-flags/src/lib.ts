import type { Flag, ParsedFlag, RegisterFlag, PrintDefaultsFn, ParseFn } from './lib.types';

const registered: Map<string, Flag> = new Map();
const parsed: Map<string, ParsedFlag> = new Map();

let isParsed = false;

const registerNumber: RegisterFlag['Number'] = (name, value, description) => {
...
const registerString: RegisterFlag['String'] = (name, value, description) => {
...
const registerBool: RegisterFlag['Boolean'] = (name, value, description) => {
...
const registerDate: RegisterFlag['Date'] = (name, value, description) => {
...
const printDefaults: PrintDefaultsFn = () => {
...
const parse: ParseFn = () => {
  ...
  for (let argIdx = 0; argIdx < args.length; argIdx++) {
    ...
    if (current.startsWith('---')) throw new Error('Please use -flag or --flag');
    for (let charIdx = 0; charIdx < current.length; charIdx++) {
      const char = current[charIdx];
      if (char === '-' && flag === '') continue;
      if (char === '=') {
    ...
    if (registeredFlag !== undefined && parsedFlag !== undefined) {
      ...
      if (registeredFlag.type === 'date') {
        ...
        if (Number.isNaN(v)) throw new Error(`Flag -${flag} is of type ${registeredFlag.type}.\nProvided: ${flagArg}`);
